...
def main(args):
    ...
    if args.mode == 'train':
        ...
        optimizer = th.optim.Adam(mlp.parameters(), lr=1e-2)
        early_stopping = EarlyStopping(patience = 10000,checkpoint_path=mlp_flie)
        for epoch in range(100000):
            mlp.train()
            predict_flow,emb = mlp(x_train_tensor)
            ...
            mlp.eval()
            print("start eval")
            predict_flow,embed = mlp(x_valid_tensor)
